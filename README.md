...
> **当前版本**: v5.0（Harness 护栏 + Token 经济学优化 + 方向三全部完成）
> **最后更新**: 2026-07-07
...
| **方向三：Skill 与提示词优化** | T0~T10 共 11 项（防幻觉规则 + 结构化输出） | ✅ 已完成（2026-06-30） |
| **🆕 Harness 护栏与 Token 经济学** | H0~H8（双环控制 + KV-Cache 优化） | ✅ 已完成（2026-07-07） |

> 历史 README（v4.0，2026-06-30）已更新为 v5.0，以 2026-07-07 现网代码为准。

---

## 🎯 核心能力

### 1. 自然语言操作（Plan-Execute + Harness 护栏）
文字/图文输入 → LLM 动态规划 → **行动前护栏校验** → 统一调用工具 → **行动后结果核对** → 汇总响应。
...
- **Planner**：从 Skill Registry 动态读取可用工具，生成 JSON 任务数组
  ...
  - **🆕 行动前 plan 硬校验**（H6）：校验工具存在性 / event_type 合法性 / 参数越界自动修正
- **Executor**：通过 `registry.invoke()` 统一调用（MCP / 本地 / 子图），支持复杂步骤间传参
  ...
  - **🆕 危险回写确认门**（H7）：update_alarm_status 等修改生产数据的操作，区分复判链路自动化 vs 用户直接回写
- **Formatter**：智能压缩中间结果（剥离 `image_base64` 等大对象为占位符，避免超 token），统一回答骨架
  - **🆕 行动后统一核对**（H8）：isError 识别 + 空结果守卫收敛，防编造数据

**Harness 双环控制**（对应《从 Harness 架构到 Token 经济学的探索》）：
- **前馈控制**（开环）：planner_prompt 防幻觉规则 + event_types 权威字典 → 行动前注入约束
- **🆕 反馈控制**（闭环）：PreToolUse 拦截（H6/H7）+ PostToolUse 核对（H8）→ 行动前后双重保障
...
│   ├── planner_prompt.py        # ✨T2 提示词分层（L1/L2/L3）+ 🆕H1 稳定前缀前置
│   ├── harness_guard.py         # 🆕H5-H8 Harness 护栏（行动前后双重保障）
...
├── eval/                        # ✨T0 轻量 planner 层回归评测 + 🆕H0 标尺
│   ├── cases.py                 # 26 条评测用例（12 维度）
│   ├── run_planner_eval.py      # 主入口（断言 + baseline diff）
│   └── baseline.json            # 基线快照
├── tests/                       # 测试
│   ├── test_harness_guard_h6.py # 🆕H6 单元测试（plan 硬校验）
│   ├── test_harness_guard_h7.py # 🆕H7 单元测试（危险回写确认门）
│   └── test_harness_guard_h8.py # 🆕H8 单元测试（行动后统一核对）
├── web/
...
## 🛡️ Harness 护栏与 Token 经济学优化（H0~H8，已全部完成）

依据《智能体平台 Harness 与 Token 经济学优化执行方案》（2026-07-03），8 项任务全部落地：

### 方向 A：Token 经济学 / KV-Cache 优化

| 任务 | 内容 | 关键产出 | 状态 |
|------|------|---------|------|
| **H0** | 标尺先行：复用 T0 回归 | `eval/` 复用，验收标准：26/26 无退化 | ✅ |
| **H1** | 修复提示词拼装顺序 | 稳定前缀真正前置（`planner_prompt.py`） | ✅ |
| **H2** | 删除死代码 | `nodes.py` catalog_block 未使用代码删除 | ✅ |
| **H3** | 开启 vLLM 前缀缓存 | `--enable-prefix-caching`（运维协同待排期） | 📝 待运维窗口 |

**核心价值**：
- H1 修复拼装顺序后，稳定前缀可被 vLLM 缓存，减少重复 prefill
- H2 删除每次规划白算的 ~600 tokens 死代码
- H3 开启后结合 H1，prefix cache 命中率提升，降低 planner 时延

### 方向 B：反馈控制 / Hooks 护栏

| 任务 | 内容 | 关键产出 | 状态 |
|------|------|---------|------|
| **H5** | 护栏骨架 | `graph/harness_guard.py`：GuardResult 统一结构 + 三护栏函数 + 三挂载点 | ✅ |
| **H6** | plan 行动前硬校验 | 工具存在性 / event_type 合法性 / pagesize 越界自动修正 | ✅ |
| **H7** | 危险回写确认门 | 区分复判链路自动化（source=vlm_judge）vs 用户直接回写（拦截） | ✅ |
| **H8** | 行动后统一核对 | isError 识别 + 空结果守卫收敛（Facade 模式），启动期自检 | ✅ |

**防幻觉链路（完整闭环）**：
```
Planner 生成 plan
  ↓
T1 guided_json：保证 JSON 结构合法
  ↓
_normalize_alarm_plan：补全缺失字段
  ↓
【前馈控制】planner_prompt 防幻觉规则 + event_types 权威字典
  ↓
H6 check_plan：行动前 plan 硬校验（PreToolUse）
  ├─ 工具存在性
  ├─ event_type 合法性
  └─ 参数越界修正
  ↓
H7 needs_confirmation：危险回写确认门（PreToolUse）
  ├─ 复判链路自动化放行
  └─ 用户直接回写拦截
  ↓
Executor 执行
...
H7 审计日志：危险工具强制留痕
  ↓
【反馈控制】H8 check_result：行动后统一核对（PostToolUse）
  ├─ isError 识别
  ├─ 空结果守卫（ai_event_list / aggregate_alarms）
  └─ 未覆盖工具默认放行
  ↓
Formatter 确定性分支 / LLM 生成回答
```

**理论映射**：
- 对应《从 Harness 架构到 Token 经济学的探索》Part 1-2 控制论双环
- 前馈控制（开环）：planner_prompt 规则注入
- 反馈控制（闭环）：PreToolUse Hooks（H6/H7）+ PostToolUse Hooks（H8）

**配置开关**（可回滚）：
```yaml
harness:
  guard_enabled: true                    # 护栏总开关（关闭则等价 H5 前）
  confirm_dangerous_writes: true         # H7 危险回写确认门子开关
  dangerous_tools: ["update_alarm_status", "ai_event_deal"]
```

**验收结果**：
- 单元测试：H6 (8/8) + H7 (8/8) + H8 (8/8)
- 集成测试：H7 (5/5)
- T0 回归：H6/H7/H8 均 26/26 通过，0 退化

详见 `plan/TASK_H5~H8_COMPLETION_REPORT.md` 及 `plan/HARNESS_TOKEN_ECONOMICS_EXECUTION_PLAN_2026-07-03.md`。
...
- `HARNESS_TOKEN_ECONOMICS_EXECUTION_PLAN_2026-07-03.md` — 🆕 Harness 护栏与 Token 经济学执行方案
- `LATENCY_OPTIMIZATION_EXECUTION_PLAN_2026-06-22.md` — P0 时延优化方案
- `TASK_T0~T10_*.md` — 方向三各任务完成报告
- `TASK_H0~H8_*.md` — 🆕 Harness 护栏各任务完成报告
...
**项目维护**: KSAgent 开发团队
**最后更新**: 2026-07-07
**版本**: v5.0（真实平台 + RAG + 记忆 + P0 时延 + 方向三 + **Harness 护栏与 Token 经济学** 全部完成）
